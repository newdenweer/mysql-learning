...
const getTasks = async (req, res) => {
	try {
		...
		if (id) {
			...
			const commentsData = await db.promise().query('SELECT text, user_id FROM comments WHERE task_id = (?)', id);
			const checklistData = await db
				.promise()
				.query('SELECT id, name, text, status, position FROM checklists WHERE task_id = (?)', id);
...
module.exports = { taskCreation, getTasks };
